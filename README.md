...
> Menu do admin: Visão geral (Dashboard, Relatórios), Operação (Agenda, Vendas, Assinaturas), Financeiro (Fluxo de caixa), Gestão (Funcionários, Cadastros) e Sistema (Aparência, Configurações).
...
- **Bloqueio de cliente**: admin bloqueia um cliente com motivo (opções padrão + "Outros" com texto livre) e prazo opcional (vazio = permanente). Não impede login, mas trava agendar e contratar planos (cancelar plano existente segue liberado); ao tentar, o cliente recebe a mensagem com motivo e prazo. Estado atual em `profiles` (enforcement rápido) + trilha em `bloqueios`. O histórico dos episódios fica no relatório **Relatórios → Bloqueios** (não na listagem de usuários).
- **Finalização de atendimento**: ao finalizar na agenda, um modal permite adicionar serviços extras (cobertura de plano reavaliada), registrar produtos vendidos (que ficam **vinculados ao atendimento** via `agendamento_id`) e escolher o **método de pagamento** (Dinheiro/Pix/Débito/Crédito). Reflete em agenda, vendas, dashboard e relatórios ("Recebimentos por método").
- **Venda avulsa vs. em atendimento**: venda de produto pela tela de Vendas é **avulsa** (sem vínculo); venda no modal de finalização é **do atendimento**. O **ticket médio** = `(serviços + produtos do atendimento) ÷ atendimentos`, deixando avulsa de fora; as avulsas são reportadas à parte (dashboard, Financeiro e Produtos).
- **Fluxo de caixa** (`/admin/caixa`): visão consolidada por período — entradas automáticas (serviços finalizados, produtos, assinaturas iniciadas), saídas automáticas (estornos) e **lançamentos manuais** de entrada/saída (despesas, retiradas, aportes) com CRUD, em KPIs de Entradas/Saídas/Saldo. Cortesia (assinatura gratuita) não entra no faturamento/receita recorrente.
- **Média de frequência e recência**: intervalo médio entre visitas e "dias sem retornar" por cliente (a partir de atendimentos finalizados + compras), na listagem de usuários, no dashboard e no relatório de Atendimentos.
- **Aparência** (`/admin/aparencia`, Sistema): tema do sistema — **Escuro** (padrão), **Claro** ou **Personalizado** (5 cores-base com o resto da paleta derivado e aviso de contraste WCAG) — e **fontes de corpo e título** separadas. `ColorPicker` próprio (paleta + spectrum), preview ao vivo. Config global em `estabelecimento_info.aparencia`, aplicada no `:root` do layout raiz (vale para vitrine e admin). Tokens de cor são CSS variables (`--c-*`) consumidas pelo Tailwind.
- **Categorias de serviço** (aba em Cadastros): categoria com `nome`, `ativo` e `ordem`. Cada serviço aponta para uma categoria (opcional) e tem uma `ordem`. No **agendamento**, os serviços aparecem agrupados por categoria (ordenadas por `ordem`, serviços por `ordem`), com grupo **"Outros"** ao fim para os sem categoria. Categoria inativa não agrupa; excluir categoria deixa os serviços sem categoria (FK `ON DELETE SET NULL`).
